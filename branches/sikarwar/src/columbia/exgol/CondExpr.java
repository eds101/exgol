        ...
        for(int i=0;i<proximity.size();i++){
            ...
            for (int xi=x1; xi <= x2; xi++){
                if(cellClass.equals(PEER)){
                    if(cells[xi][y1].state.equals(cells[x][y].state)) cellCount++;
                    if(cells[xi][y2].state.equals(cells[x][y].state)) cellCount++;
                }
            }
            for (int yi=y1+1; yi <= y2-1; yi++){
                if(cellClass.equals(PEER)){
                    if(cells[x1][yi].state.equals(cells[x][y].state)) cellCount++;
                    if(cells[x2][yi].state.equals(cells[x][y].state)) cellCount++;
                }
            }
        }
        return cellCount;
